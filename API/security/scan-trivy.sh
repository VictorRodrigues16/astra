...
#    2) IMAGEM DOCKER        -> trivy image (SO + libs do node:14)      -> V3
#  Bonus (mesma ferramenta, custo zero):
#    - secret scan           -> segredos hardcoded (.env, config.js)    -> V1
#    - misconfig (IaC)       -> Dockerfile inseguro (root, .env, etc.)  -> V3
#
...
IMAGE_NAME="${IMAGE_NAME:-astra-api:vuln}"       # imagem a escanear
SEVERITY="${SEVERITY:-HIGH,CRITICAL}"            # severidades reportadas (gate padrao da industria)
...
echo " Severidade: $SEVERITY | Gate (exit-code): $EXIT_CODE"
echo "============================================================"
...
# OBS: o JSON completo da imagem nao e gerado por padrao porque o node:14 tem
# ~1300 CVEs e o arquivo passa de 5 MB (poluiria o repositorio). Se precisar do
# JSON da imagem, rode manualmente:
#   IMAGE_NAME=astra-api:vuln SEVERITY=HIGH,CRITICAL \
#   docker run --rm -v "$PWD/security/.trivycache":/root/.cache/trivy \
#     aquasec/trivy:latest image --format json astra-api:vuln > security/reports/02-image.json
